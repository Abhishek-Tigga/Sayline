...
/// The floating pill (Figma node 36:656/657/688,
/// fnMh8Ujn4OxhdW9IKNFaH9): a four-dot processing glyph plus a status
/// word ("Listening" / "Agent Listening"). No waveform, no app-icon logo
/// box — replaced an earlier logo+waveform design (still available on
/// the `ui-redesign-v3` branch if ever needed for reference).
///
...
/// IS the glass look, so it doesn't cleanly separate from "looks like
/// glass" while still calling the real API. Explicit call (2026-08-07):
/// keep the real glass, flicker included, rather than any of the
/// workarounds — needs a real solution, not a patch, revisit before
/// this ships.
///
...
/// variants, 3 text-breathing options) before porting.
struct RecordingIndicatorView: View {
    ...
    /// combined value (no intermediate background between the two, so
    /// nothing is lost).
    ...
    var body: some View {
        Group {
            if #available(macOS 26.0, *) {
                ...
                // Pre-macOS 26 fallback — real Liquid Glass isn't
                // reachable below macOS 26.
                pillContent
                    .padding(.horizontal, Self.horizontalPadding)
                    .padding(.vertical, Self.verticalPadding)
                    .background(
        ...
        // read badly) at .mono/40% strength, so the two states still
