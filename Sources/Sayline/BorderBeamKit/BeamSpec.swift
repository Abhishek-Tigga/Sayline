...
struct BeamSpec: Decodable {
    ...
    ///
    /// Vendored directly into the Sayline target rather than pulled in as an
    /// SPM package (upstream's Package.swift isn't at the repo root, so
    /// `.package(url:)` doesn't resolve — see BorderBeamKit/LICENSE-BorderBeamKit.txt).
    /// `Bundle.module` only exists for a real SPM target build, so this one
    /// line is changed from upstream to `Bundle.main`, same as how
    /// AppIconCatalog loads claude.svg.
