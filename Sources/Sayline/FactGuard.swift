...
enum FactGuard {
    ...
    static func extract(from raw: String) -> FactSet {
        ...
        facts.numbers = SpokenNumbers.numbers(in: words)
    ...
    static func verify(raw facts: FactSet, rewrite: String,
        ...
        let present = SpokenNumbers.numbers(in: words)
    ...
    static func retractionDropSpan(raw: String, cleaned: String) -> Set<Int> {
        ...
        let numberPositions = words.indices.filter { SpokenNumbers.numberPhrase(in: words, at: $0) != nil }
        ...
        for marker in markers {
            ...
            for positions in [dayPositions, namePositions, numberPositions] {
                ...
                var end = retracted + 1
                if let (_, next) = SpokenNumbers.numberPhrase(in: words, at: retracted) { end = next }
    ...
    private static func withoutRetractions(_ violations: [Violation],
        ...
        let rewriteNumbers = SpokenNumbers.numbers(in: rewriteWords)
        let rawNames = properNouns(in: raw)
        ...
        for violation in violations {
            ...
            case .numberLost(let value):
                let all = rawWords.indices.filter { !SpokenNumbers.numbers(in: [rawWords[$0]]).isEmpty }
                let mine = all.filter { SpokenNumbers.numbers(in: [rawWords[$0]]).contains(value) }
                let survived: (String) -> Bool = { word in
                    !SpokenNumbers.numbers(in: [word]).isEmpty
                        && !SpokenNumbers.numbers(in: [word]).isDisjoint(with: rewriteNumbers)
                }
    ...
    private static func units(in text: String, words: [String]) -> Set<String> {
        ...
        where unitWords.contains(word) && SpokenNumbers.scaleWords[word] == nil {
            ...
            let isQuantified = neighbours.contains { token in
                Int(token) != nil || SpokenNumbers.spokenUnits[token] != nil
                    || SpokenNumbers.spokenTens[token] != nil || SpokenNumbers.quantityWords[token] != nil
                    ...
                    // an invented one.
                    || SpokenNumbers.scaleWords[token] != nil
            }
    ...
    // Internal, not private: `SpokenNumbers.numberPhrase` consults it for
    // the "one counts only when a unit follows" rule — the one shared
    // lexicon between the two files after the 2026-08-14 split.
